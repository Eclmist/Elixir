...
public:
    ...
    Box(const exrPoint& position, const exrVector3& scale, std::unique_ptr<Material> material)
        : Primitive(material)
    {
        m_Transform.SetTranslation(exrVector3(position.x, position.y, position.z));
        m_Transform.SetScale(scale);

        m_LocalMin = exrPoint::Zero() + (exrVector3(-0.5f) * scale);
        m_LocalMax = exrPoint::Zero() + (exrVector3(0.5f) * scale);
    ...
    //! 
    //! @return                 Always return true since bounding volumes can be created for boxes
